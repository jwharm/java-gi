...
public class ReturnValue extends Parameter {
    ...
    public void generateReturnStatement(Writer writer, int indent) throws IOException {
        ...
        if (type.simpleJavaType.equals("Type") || type.isAlias() && (!((Alias) type.girElementInstance).inherits())) {
            ...
            writer.write("return new " + type.qualifiedJavaType + "(ProxyFactory.getProxy(RESULT, " + (ownedByCaller() ? "true" : "false") + "));\n");
        } else if (type.isInterface()) {
            writer.write("return new " + type.qualifiedJavaType + "." + type.simpleJavaType + "ProxyInstance(ProxyFactory.getProxy(RESULT, " + (ownedByCaller() ? "true" : "false") + "));\n");
        } else if (type.isEnum()) {
